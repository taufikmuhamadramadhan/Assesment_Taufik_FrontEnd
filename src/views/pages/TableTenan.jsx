...
const KelolaDataDosenWali = () => {
  ...
  useEffect(() => {
    ...
    const apiUrl = 'http://localhost:8080/api/tenan/'
    ...
    axios
      ...
      .then((response) => {
        ...
        const TenanData = response.data.map((item) => {
          return {
            KodeTenan: item.KodeTenan,
            NamaTenan: item.NamaTenan,
            HP: item.HP,
          }
        })
  ...
  const handleDelete = (id) => {
    ...
    axios
      ...
      .then((response) => {
        ...
        setTenanData((prevData) => prevData.filter((barang) => barang.KodeBarang !== id))
  ...
  const filteredData = TenanData.filter((data) => {
    //Search filter data
    return (
      searchText === '' ||
      data.KodeTenan.toLowerCase().includes(searchText.toLowerCase()) ||
      data.NamaTenan.toLowerCase().includes(searchText.toLowerCase()) ||
      data.HP.toLowerCase().includes(searchText.toLowerCase())
    )
  })
    ...
    <div>
      <CRow>
        <CCol>
          <CCard>
            <CCardBody>
              <div className="fw-bold mt-3 mb-5 title-page">Daftar Tenan</div>
                ...
                <CTableBody>
                  {filteredData.length === 0 ? (
                    ...
                    filteredData.map((data) => (
                      <CTableRow key={data.KodeTenan}>
                        <CTableDataCell>{data.KodeTenan}</CTableDataCell>
                        <CTableDataCell>{data.NamaTenan}</CTableDataCell>
                        <CTableDataCell>{data.HP}</CTableDataCell>
